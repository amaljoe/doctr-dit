...
def compute_poly_iou(list1, list2):
    ...
    try:
        ...
        iou = poly1_clean.intersection(
            poly2_clean).area / poly1_clean.union(poly2_clean).area
...
class Table:
    ...
    def parse_table(self):
        # get the table bbox
        self._table_coords = str(self._root.getElementsByTagName("Coords")[
                                 0].getAttribute("points"))
        ...
        for cell in cells:
            ...
            b_points = str(cell.getElementsByTagName(
                "Coords")[0].getAttribute("points"))
            # try:
            #     try:
            ...
            er = cell.getAttribute(
                "end-row") if cell.hasAttribute("end-row") else -1
            ec = cell.getAttribute(
                "end-col") if cell.hasAttribute("end-col") else -1
    ...
    def convert_2d(self):
        ...
        table = [[0 for x in range(self._maxCol+1)]
                 for y in range(self._maxRow+1)]
                            ...
                            for cell in temp_pos:
                                ...
                                if tab[r][c_to] != 0:
                                    ...
                                    if type(tab[r][c_to]) == list:
                                        ...
                                        if cell != tab[r][c_to]:
                                            adj_relation = AdjRelation(
                                                cell, tab[r][c_to], AdjRelation.DIR_HORIZ)
                                            retVal.append(adj_relation)
                                else:
                            ...
                            if tab[r][c_to] != 0:
                                ...
                                if type(tab[r][c_to]) == list:
                                    ...
                                    if temp_pos != tab[r][c_to]:
                                        adj_relation = AdjRelation(
                                            temp_pos, tab[r][c_to], AdjRelation.DIR_HORIZ)
                                        retVal.append(adj_relation)
                            else:
                                # find the next non-blank cell, if exists
                                for temp in range(c_from + 1, self._maxCol + 1):
                                    if tab[r][temp] != 0:
                                        if type(tab[r][temp]) == list:
                                            ...
                                            adj_relation = AdjRelation(
                                                temp_pos, tab[r][temp], AdjRelation.DIR_HORIZ)
                                            retVal.append(adj_relation)
                                        break
                                ...
                                for temp in range(r_from + 1, self._maxRow + 1):
                                    if tab[temp][c] != 0:
                                        if type(tab[temp][c]) == list:
                                            ...
                                            adj_relation = AdjRelation(
                                                temp_pos, tab[temp][c], AdjRelation.DIR_VERT)
                                            retVal.append(adj_relation)
                                        break
